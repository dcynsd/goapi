...
func NewJWT() *JWT {
	return &JWT{
		SignKey:    []byte(config.Config.AppKey),
		MaxRefresh: time.Duration(config.Config.JwtMaxRefreshTime) * time.Minute,
	}
}
...
// 更新 Token，用以提供 refresh token 接口
...
func (jwt *JWT) IssueToken(userID uint64, userName string) string {
	...
	claims := JWTCustomClaims{
		...
		jwtpkg.StandardClaims{
			...
			Issuer:    config.Config.AppName,          // 签名颁发者
		},
	}
...
// 创建 Token，内部使用，外部请调用 IssueToken
...
func (jwt *JWT) expireAtTime() int64 {
	...
	if config.Config.AppDebug {
		expireTime = config.Config.JwtDebugExpireTime
	} else {
		expireTime = config.Config.JwtExpireTime
	}
